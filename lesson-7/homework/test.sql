...
select avg(age) as avg_employees from Employees
...
--9) Write a query to calculate the total sales per Customer in the Sales table.
 ...
 select * from Employees
 select DepartmentName, count ( Employeeid) as Count_Employees
 from Employees
 group by DepartmentName
...
select * from Employees
select departmentname, avg( salary)as Avg_salary, count(employeeid) as Cnt_Department
from Employees
Group by DepartmentName;
...
select * from Sales
select saledate, sum (saleamount) as Total_sales
from Sales
group by year(saledate)
order by saledate;
--18) Write a query to show the list of customers who placed at least 3 orders.
select * from Orders
select customerid, count (customerid) as Cnt_orders
from Orders
group by CustomerID
...
select * from Orders;
select orderid, sum (totalamount) as Total_amount
from Orders
group by orderid
